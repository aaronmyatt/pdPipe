  ...
  return funcs
    ...
    .map(({ func, config }, index) =>
      async function (input: I) {
        ...
        const validator: "some" | "every" = config.and.length
          ? "every"
          : "some";
        const conditions: string[][] = config.checks.map(checker).concat(
        ...
        if (conditions.length) {
          ...
          const orChecks = orConditions.some(([_key, value]: string[]) =>
            !!value
          );
